...
def visualize_flood(dem: np.ndarray, water_level: float, save_path: str = None):
    """
    Visualize flood inundation analysis with multiple plots.
    ...
    contour_levels = np.arange(dem.min() // 5 * 5, dem.max() // 5 * 5 + 5, 5)
    ...
    ax2.text(0.98, 0.02, f"Flooded: {percentage:.1f}%", transform=ax2.transAxes,
    ...
    if len(flooded_depths) > 0:
        mean_depth = flooded_depths.mean()
        ax4.axvline(mean_depth, color='red', linestyle='--', linewidth=2)
        ax4.text(0.95, 0.95, f"Mean depth: {mean_depth:.2f}m\nMax depth: {max_depth:.2f}m",
                 transform=ax4.transAxes, fontsize=10, ha='right', va='top')

    fig.suptitle(f"Flood Inundation Analysis — Water Level: {water_level}m", fontsize=14)
    plt.tight_layout()
